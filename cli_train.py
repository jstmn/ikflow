...
from src.model import ModelWrapper_MultiCINN, ModelWrapper_cINN_Softflow
from src.training_parameters import MultiCINN_Parameters, cINN_SoftflowParameters, TrainingConfig
...
python3 cli_train.py --model_type=cinn_softflow --robot=valkyrie --dataset_tag=plus_1pi_on_32_MD_SIZE --smoke_test
python3 cli_train.py --model_type=cinn_softflow --robot=panda_arm --smoke_test --log_dist_plots
python3 cli_train.py \
    --model_type=cinn_softflow \
    --robot=panda_arm \
    --nb_nodes=12 \
    --dim_latent_space=9 \
    --coeff_fn_config=3 \
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_type", required=True, type=str, help='One of ["cinn_softflow", "mdn", "iresnet", "multi_cinn"]'
    )
    ...
    assert args.model_type in ["cinn_softflow", "mdn", "iresnet", "multi_cinn"]
    if args.dataset_tag:
    ...
    if args.model_type == ModelWrapper_cINN_Softflow.nn_model_type:
