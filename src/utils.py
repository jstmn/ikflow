...
from src.model import ModelWrapper_cINN_Softflow, ModelWrapper_MultiCINN, ModelWrapper
from src.training_parameters import cINN_SoftflowParameters, MultiCINN_Parameters
...
def load_model_from_wandb(
...
) -> Tuple[str, Union[cINN_SoftflowParameters, MultiCINN_Parameters]]:
    ...
    assert run.config["model_type"] in ["cinn_softflow", "multi_cinn"]
    if run.config["model_type"] == "cinn_softflow":
...
def get_model_wrapper(run_name, wandb_run: Optional[Run] = None) -> Tuple[ModelWrapper, cINN_SoftflowParameters]:
    """Load a robot"""
    ...
    if isinstance(hyper_parameters, cINN_SoftflowParameters):
