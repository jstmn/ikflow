...
class KlamptRobotModel(RobotModel):
    ...
    def __init__(
        ...
        verbosity=0,
        end_poses_to_plot_solution_dists_for=[],
        l2_loss_pts: List[Tuple[float, float, float]] = DEFAULT_L2_LOSS_PTS,
    ):
    ...
    def visualize_kinpy(self):
        """ """
        # th = {'panda_joint1': 0.0, 'panda_joint2': .25}
        th = {}
        # for joint in self.actuated_joints:
        # 	th[joint] = np.pi / 2
    ...
    # TODO(@jeremysm): Set joints limits corresponding to the joint limit deltas used in the dataset
